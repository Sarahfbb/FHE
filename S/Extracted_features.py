...
# Load the VGG16 model and move it to device
try:
...
# Define a custom VGG feature extractor class with reduced dimension and ReLU activation
class FHEFriendlyVGGFeatureExtractor(nn.Module):
    def __init__(self, vgg_model, reduced_dim=1000):
...
test_loader = DataLoader(reduced_cifar10_test, batch_size=batch_size, shuffle=False, num_workers=2, pin_memory=True)

# Initialize the VGG feature extractor and move it to device
reduced_dim = 1000
...
numpy.save(os.path.join(features_dir, 'Train_Features.npy'), train_features)
numpy.save(os.path.join(features_dir, 'Train_Targets.npy'), train_targets)
numpy.save(os.path.join(features_dir, 'Val_Features.npy'), val_features)
numpy.save(os.path.join(features_dir, 'Val_Targets.npy'), val_targets)
numpy.save(os.path.join(features_dir, 'Test_Features.npy'), test_features)
numpy.save(os.path.join(features_dir, 'Test_Targets.npy'), test_targets)
