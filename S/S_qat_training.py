...
class QuantFHEFriendlyMLPClassifier(nn.Module):
    def __init__(
        self, 
        input_dim, 
        num_classes, 
        ...
        self.fc1 = QuantLinear(input_dim, 256, bias=False, weight_bit_width=bit, weight_quant=weight_quant, return_quant_tensor=True)
        self.bn1 = QuantBatchNorm1d(256, bit_width=bit)
        self.act1 = qnn.QuantReLU(return_quant_tensor=True, bit_width=bit, act_quant=act_quant)
        
        self.fc2 = QuantLinear(256, 128, bias=False, weight_bit_width=bit, weight_quant=weight_quant, return_quant_tensor=True)
        self.bn2 = QuantBatchNorm1d(128, bit_width=bit)
        self.act2 = qnn.QuantReLU(return_quant_tensor=True, bit_width=bit, act_quant=act_quant)
        
        self.fc3 = QuantLinear(128, num_classes, bias=True, weight_bit_width=bit, weight_quant=weight_quant, return_quant_tensor=True)
...
def train_qat(model, train_loader, val_loader, param, device):
    ...
    for epoch in range(param['epochs']):
        ...
        if val_accuracy > best_val_accuracy:
            ...
            torch.save(model.state_dict(), 'Best_Fhe_friendly_qat_mlp_model.pth')
            print(f"New best model saved with validation accuracy: {best_val_accuracy:.2f}%")

    # Save the final model
    torch.save(model.state_dict(), 'Final_Fhe_friendly_qat_mlp_model.pth')
    return model
...
def main():
    try:
        ...
        best_model_state = torch.load('Best_Fhe_friendly_qat_mlp_model.pth', map_location=DEVICE)
        qat_mlp.load_state_dict(best_model_state)
        qat_mlp.to(DEVICE)  # Ensure the model is on the correct device after loading
